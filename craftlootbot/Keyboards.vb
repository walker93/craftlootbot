...
Module Keyboards
    ...
    Function creaConfrontaKeyboard(Optional withzaino As Boolean = False) As ReplyMarkups.ReplyKeyboardMarkup
        Dim keyboard As New ReplyKeyboardMarkup
    ...
    Function creaErrorInlineKeyboard() As InlineKeyboardMarkup
        Dim keyboardbuttons()() As InlineKeyboardButton
        Dim button1 As New InlineKeyboardButton() With {.Text = "CANCELLA DATI", .CallbackData = "err_reset"}
        Dim button2 As New InlineKeyboardButton() With {.Text = "Non ripristinare i dati", .CallbackData = "DelMess"}
        Dim row1() As InlineKeyboardButton
    ...
    Function creaCraftKeyboard(ids() As Integer, UserID As Long) As ReplyMarkups.InlineKeyboardMarkup
        ...
        Dim button As New InlineKeyboardButton() With {.Text = "File di testo", .CallbackData = "craftF_" + filename}
        Dim button2 As New InlineKeyboardButton() With {.Text = "Messaggio", .CallbackData = "craftM_" + filename}
    ...
    Function creaInfoKeyboard(ids() As Integer) As InlineKeyboardMarkup
        ...
        For Each i In ids
            Dim button As New InlineKeyboardButton() With {.Text = ItemIds(i).name, .CallbackData = "info_" + i.ToString}
            row.Add(button)
            If row.Count = 2 Then
                keyboardbuttons.Add(row.ToArray)
                row.Clear()
